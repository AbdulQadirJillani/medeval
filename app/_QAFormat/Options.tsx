    ...
    <div className="flex flex-col gap-[0.6rem]">
      {
        sortedOptions.map((val, optionIndex) => (
          <button tabIndex={1} className={cn("focus:outline-none focus-visible:ring text-left py-3 px-5 text-lg bg-accent rounded-lg cursor-pointer", clickedOption.map(obj => ((obj.questionIndex == questionIndex && obj.optionIndex.includes(optionIndex)) ? (val.bool ? "bg-green-500" : "bg-red-500") : "")))} onClick={() => optionClick(questionIndex, optionIndex, val)} key={val.option}>
            {val.option}
